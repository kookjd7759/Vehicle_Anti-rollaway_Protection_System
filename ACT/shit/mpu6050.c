...
static uint8 mpu6050WriteReg(uint8 reg, uint8 value)
{
    ...
    if (IfxI2c_I2c_write(&g_mpu6050Dev, buf, 2) != IfxI2c_I2c_Status_ok)
        return 0U;
    return 1U;
}
...
{
    if (IfxI2c_I2c_write(&g_mpu6050Dev, &reg, 1) != IfxI2c_I2c_Status_ok)
        return 0U;
    if (IfxI2c_I2c_read(&g_mpu6050Dev, pData, len) != IfxI2c_I2c_Status_ok)
        return 0U;
    return 1U;
}
